...
opg.api({
	...
	audit: 'audit/findPage?stepCode=audit',
		...
		},
		{
			...
			render: function (val, i, row) {
				if (!row.executor || (row.executor == currentUser.loginName))
					return `<button class="btn-mini btn-warning" data-oid="${val}" data-aid="${row.assetId}" data-title="${row.managerName}" data-idx="${row[':index']}">四审</button>`;
				//return '';
			...
			let popWin = openInfoWindow(assetId, idx, {
				...
				buttons: {
					...
					btnPutBack: {
						...
						text: '打回',
						onClick: function () {
							AuditPutBack.showWindow(4 , orderId , title , popWin , tb);
							return true;
						}
					},
					ok: {
